...
import { useWingspanStore } from '@/stores/wingspan'
import { computed } from 'vue'

const store = useWingspanStore()
const { createWingspanGame, loadWingspanGame } = store
...
function handleReset() {
  if (confirm('Aktuelles Spiel zurücksetzen?')) {
    store.resetCurrentWingspanGame()
  }
}
</script>
    ...
    <div class="max-w-md mx-auto flex items-center justify-between gap-4">
      <div class="flex items-center gap-2">
        <select
          ...
          @change="loadWingspanGame(($event.target as HTMLSelectElement).value)"
        >
